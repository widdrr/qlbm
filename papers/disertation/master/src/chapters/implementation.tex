...
\end{equation}
with $w_i>0$ being the link weight, $c_i$ being the link velocity and $c_s$ a constant representing the speed of sound in the medium.
...
The full derivation can be found in Appendix \todo{decide to add this or not}

Since we encode $\Phi$ in the uniform superposition of the initial state, we can be implement the collision as a single diagonal matrix $M$ of size $2^{m+n} \times 2^{m+n}$, where $m$ is the number of lattice qubits and $n$ is the number of link qubits.
\begin{equation*}
    M = \begin{pmatrix} 
    ...
    0 & 0 & \dots & w_{Q-1} \left( 1 + \frac{\mathbf{c}_{Q-1} \cdot \mathbf{u}(\mathbf{x}_N,t)}{c_s^2} \right) 
    \end{pmatrix}
\end{equation*}
...
Fortunately we can implement this as a block encoding, by writing
$M$ as a linear combination of unitaries $\frac{1}{2}(U_0 + U_1)$ with $U_0 = M + i\sqrt{I - M^2}$ and $U_1 = M - i\sqrt{I - M^2}$.
\todo{detailed derivation in appendix or here?}
