...
Our QLBM algorithm is heavily based on the work of Wawrzyniak et al. \cite{wawrzyniak2025base}, which in turn builds upon Budinski's original algorithm.
Their version extends the original algorithm to support non-uniform velocity fields and improves the initialization procedure.
...
\begin{enumerate}
    ...
    \item \textbf{Computing Macroscopic Quantities}: Recover the macroscopic distribution from the final state.
\end{enumerate}
...
Since the transpilation of the quantum circuit is the bottleneck of an iteration, instead of performing it each time, we transpile all \emph{distinct configurations} upfront and reuse the circuits during the simulation.
...
Following Wawrzyniak et al. \cite{wawrzyniak2025base}, we only initialize one of the link substates like this and make use of Hadamard gates to prepare the remaining substates.
...
We used Qiskit's \texttt{DiagonalGate} to implement these unitaries.


The LCU circuit is identical to the one in Figure~\ref{fig:lcu_circuit}, noting that because both coefficients are equal,
...
The basic operation is shifting the amplitudes of a quantum state left or right, denoted as $LGate$ and $RGate$ respectively in Figure~\ref{fig:streaming_gates}.
...
This can be done either classically, after obtaining the final state, or by applying a series of Hadamard and Swap gates to accumulate the distributions into a single link substate.

Our implementation uses the quantum variant. An example of a complete circuit is show in Figure~\ref{fig:complete_circuit}
...
We could not find an existing classical simulator that would produce an output in a similar format, so we implemented a basic one.
...
All code is open source and available on GitHub~\cite{qlbm2025github}
...
This experiment also shows an exact match, with an RMSE on the order of $10^{-14}$.
...
This configuration yielded the highest error out of all the experiments at around $10^{-10}$.
The higher error cannot be entirely explained by a larger circuit depth compounding floating-point errors.
Instead, it is caused by defensive guards against small floating-point values during Qiskit's transpilation process.
For a more detailed analysis, see Appendix~\ref{app:rmse_analysis}. 
In any case, it is not an inherent flaw in the theoretical formulation of the algorithm 
...
You may notice that the final 10 iterations have parameters that no longer recover the ADE.
...
The most striking characteristic of this experiment's RMSE plot is the decrease in error once the velocity field is removed at iteration $15$.
This phenomena can be explained due to the dissipative nature of pure diffusion: any noise will smooth out as the distribution relaxes to equilibrium.
...
This complexity does not depend on the specifics such as the velocity field.

For our QLBM we already established in the previous section that the total number of qubits used is $n+m+1$. Writing the sum in terms of $M$ and $N$ results in a logarithmic complexity: $\mathcal{O}(\log{M} + \log{N})$.
Since we are encoding information in superposition states, it's natural to see an improvement here.

Analyzing the time complexity is not so straightforward.
First of all, in a quantum circuit we are usually interested in the \emph{circuit depth}: the number of sequential quantum gates used to implement the circuit as a function of the size of the input.
When counting this depth, gates applied "in parallel" on different qubits can be considered constant time.
In addition, we only count elementary gates, i.e., the aforementioned \texttt{DiagonalGate} is not constant time, but perhaps exponential depending on the gate decomposition.
...
Since we only initialize the $m$ lattice qubits, this results in a time complexity of $\mathcal{O}(2^m)$ or $\mathcal{O}(M)$.
The $H$ gates applied on the link qubits are $\mathcal{O}(1)$.
...
For a diagonal gate in particular, the base of the exponential is $2$.
Since we are manually computing the controlled unitaries, each matrix is $n+m+1 \times n+m+1$, meaning the final worst case complexity for the collision stage is $\mathcal{O}(2^{(m+n)})$ or $\mathcal{O}(M\cdot N)$.
...
Let's denote the average number of circuits applied per link velocity with $q$.
...
\paragraph*{Macroscopic Quantities Computation} is linear with respect to the number of links: $\mathcal{O}(N)$; as previously described.

Since all of these steps are in sequence, the total complexity is dominated by either the initialization or the collision step.
...
In addition to the theoretical analysis, we have performed a concrete numerical analysis for the gate count and circuit depth of each stage under various configurations.
...
Another contributing factor is that at low $N$ and $M$ values, constants dominate the complexity.
...
In Table~\ref{tab:init_per_iter} we counted the total gates and circuit depth for each iteration as the distribution evolves for a given configuration.
Notice how the first iteration has a noticeably lower circuit depth; this is due to the extra structure of the initial distribution (a gaussian ring).
As the simulation evolves and the distribution becomes more chaotic, the depth increases but quickly stabilizes.
