...
\end{equation}
with $w_i>0$ being the link weight, $c_i$ being the link velocity and $c_s$ a constant representing the speed of sound in the medium \cite{timm2016lattice}.
...
\todo{consider moving this to other section to new chapter at the end}
...
Given a lattice with a total of $M=\sum_{i=0}^{d}M_i$ sites and $N$ propagation directions, the complexity is $\mathcal{O}(M\cdot N)$ \cite{timm2016lattice}.
...
\midrule
D1Q3 $M{=}4$ + BC & 5 & 86 / 81 & 6 / 3 & 2,282 / 1,432 & 101 / 69 & 6 / 3 & \textbf{2,481 / 1,588} \\
D1Q3 $M{=}8$ + BC & 6 & 129 / 118 & 6 / 3 & 9,451 / 6,008 & 663 / 491 & 6 / 3 & \textbf{10,255 / 6,623} \\
D1Q3 $M{=}16$ + BC & 7 & 172 / 155 & 6 / 3 & 38,380 / 24,568 & 1,947 / 1,453 & 6 / 3 & \textbf{40,511 / 26,182} \\
\midrule
...
D2Q5 $4{\times}4$ + BC & 8 & 364 / 347 & 9 / 3 & 154,610 / 99,320 & 282 / 208 & 9 / 3 & \textbf{155,274 / 99,881} \\
\midrule
...
The QLBM configuration used was a standard D1Q3 with 32 lattice sites, simulated for 50 iterations.
The link set is $\{(0), (-1), (1)\}$ with weights $\{2/3, 1/6, 1/6\}$.
The initial distribution is a Gaussian hill of amplitude 0.9 centered at $x=8$ with width $\sigma=2$. The background density is 0.1.
The velocity field is a constant $u=0.3$ causing a rightward advection.
...
This experiment showed exact agreement between the quantum and classical simulations, with the RMSE in the order of $10^{-14}$ attributed to machine precision.

\paragraph*{2D Simulation}
The QLBM configuration used was a standard D2Q9 with $32 \times 16$ lattice sites, simulated for 40 iterations.
The link set is $\{(0,0), (\pm1, 0), (0, \pm1), (\pm1, \pm1)\}$ with weights $4/9$ for the stationary, $1/9$ for the orthogonal and $1/36$ for the diagonal links.
The initial distribution is a ring of amplitude $0.9$ centered in the grid, with outer radius $4$, inner radius $2$. The background density is $0.1$.
The velocity field consists of a constant $u = 0.2$ in the bottom half and $u=-0.2$ in the top half with a constant $v=0.1$, creating a shear field.
...
Once again, the numerical differences are negligible at $10^{-14}$.

\paragraph*{3D Simulation}
The QLBM configuration used was a standard D3Q27 with $8 \times 8 \times 8$ lattice sites, simulated for 10 iterations.
The link set is the rest vector, the six axial neighbors, the twelve edge neighbors and the eight corner neighbors, with weights $8/27$, $2/27$, $1/54$ and $1/216$ respectively.
The initial distribution is three single-cell perpendicular planes through the center of the grid, with density $0.9$ on the planes and background density $0.1$ elsewhere.
The velocity field follows the pattern:
...
\end{figure}
This simulation shows the highest RMSE of around $10^{-4}$.
This is attributed to the more complex velocity field, but I have yet to find the exact explanation.
\todo{investigate further}

\paragraph*{2D Simulation with Non-Standard Link Velocities}
The number of lattice sites is $32 \times 32$.
The initial distribution is a cross with lines $4$ cells wide, with density $0.9$ on the cross and background density $0.1$ elsewhere.

The configuration changes during the simulation in the following manner:
\begin{itemize}
    \item For the first 15 iterations, the standard D2Q9 link set and weights ($4/9$, $1/9$, $1/36$) are used and the velocity field is a clockwise rotation vortex of magnitude $u=0.5$.
    \item For the next 10 iterations, the velocity field disappears and the link velocities are changed to the non-standard set $\{(0,0), (2,1), (-2,-1)\}$ with weights $\{2/3, 1/6, 1/6\}$, restricting propagation to a single diagonal direction.
\end{itemize}

You may notice that the the final 10 iterations have parameters that no longer recover the ADE.
Indeed this experiment no longer models the usual phenomena for which the equation was designed, but this does not make the experiment invalid.
...
Such behavior is called \emph{anisotropic transport}.
...
The RMSE for this simulation is also considerably higher, at $10^{-5}$. The more interesting part is that once the velocity field disappears at $t=15$, the error starts to decrease, as seen in Appendix \ref{app:rmse}.
